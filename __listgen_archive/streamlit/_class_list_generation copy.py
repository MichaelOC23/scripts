...
#*################################################################    
    ...
    def extract_N_urls_from_markdown(self, markdown, N=5):
        try:
            ...
            #%print(f"An error extracting URLs from markdown occurred: {e}")
            return []
...
#*################################################################ 
    ...
    async def clean_name_list_gemini(self, name_list):
        ...
        try :
            ...
            print(f"1st attempt complete in seconds is {time.time() - start_time}")
            return json.loads(response.text)
        except:
            try :
                response = await get_response(prompt)
                print(f"2nd attempt complete in seconds is {time.time() - start_time}")
                return json.loads(response.text)
            except:
        ...
        for key in base_row.keys():
            ...
            if isinstance(base_row.get(key), dict):
                ...
                if isinstance(new_row.get(key), dict):
                    ...
                    #%print(f"Error updating field {key}: cannot insert/update a dict value {new_row.get(key)} into a non-dict field with value: {base_row.get(key)}")
                    continue
...
#*################################################################
    ...
    async def async_scrape_single_url(self, url):
        ...
        async def extract_same_domain_urls(page, base_url):
            ...
            #%print(f"Found {len(anchor_elements)} anchor elements")
        ...
        try:
            async with async_playwright() as p:
                ...
                #%print(f'return_dict complete for {url}: \n {return_dict}')
                await browser.close()
                print(f"\033[1;32mSCRAPE SUCCESSFUL: Duration: {datetime.now() - scrape_start_time} seconds for url: {url}\033[0m")
        ...
        #     "NC":"\033[0m'"} # No Colo"}
        # "RED":"\033[1;31m",
            print(f"\033[1;31mAfter {datetime.now() - scrape_start_time} seconds, the scrape failed for {url} with error: {str(e)}\033[0m")
            return return_dict
        
        except Exception as e:
            #%print(f"An unexpected error occurred during the Playwright scraping: {str(e)}")
    ...
    async def google_search(self, query, num_results=10):
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                # print(f"Google Search API response: {response}")  # Log response for debugging
        ...
        try:
            ...
            print("Invalid JSON response from Google Search API")
...
#!################################################################
    ...
    async def search_for_executives(self, scrape_dict ):
        ...
        print(f"Searching for executives with query: {query}")
    ...
    async def scrape_seed_urls(self, scrape_dict, next_processing_status='SCRAPE1'):
        ...
        url_list = [url for url in scrape_dict.get('urls', []) if url is not None]
        url_for_print = "\n---->  ".join(url_list[:10])
    ...
    async def extract_names(self, text):
        ...
        """
        print("Extracting names with NLTK...")
        start_time = time.time()
        ...
        print(f"Extracted {len(deduped_names)} names in {time_taken:.2f} seconds.")
        likely_names = []
    ...
    async def get_framework_from_gemini(self, scrape_dict):
        ...
        get_responses = await self.process_text_blocks(raw_text, prompt, max_tokens=4000, model="nuextract")
        # get_responses = [ask_gemini(sentence) for sentence in sentences if sentence is not None and sentence != ""]
                ...
                if start_processing_status == 'NEW':
                    ...
                    if emails and len(emails) > 0:
                        try: 
                            ...
                            main_url = scrape_dict.get('urls', '')[0]
                            main_domain = tldextract.extract(main_url).domain   

                            emails_main = []
                            emails_other = []
                            for email in email_dict.get('emails', []):
                                if main_domain in email:
                                    emails_main.append(email)
                        
                                else:
                                    emails_other.append(email)
                    
                
                            scrape_dict['emails'] = {main_domain: emails_main, "Other": emails_other}
                        except Exception as e:
                            ...
                            if scrape_dict.get('notes', []) is None: scrape_dict['notes'] = []
    ...
    async def get_bypage_list_safely(self, scrape_dict):
        def get_bypage_value_safely(bypage_value):
            bp_text =""
            if isinstance(bypage_value, dict):
                for key, value in bypage_value.items(): 
                    bp_text += f"{'-' * 30}\n{key}\n{value}\n\n"
                return bp_text
            count = 0
            while count<=10:
                try:
                    bypage_value = json.loads(bypage_value)
                    if isinstance(bypage_value, dict):
                        for key, value in bypage_value.items(): 
                            bp_text += f"{'-' * 30}\n{key}\n{value}\n\n"
                        return bp_text 
                    elif isinstance(bypage_value, list):
                        bp_text += f"{"\n\n".join(bypage_value)}\n\n"
                        return bp_text 
                    else: count+=1
                except: return f"{bypage_value}"
            return f"{bypage_value}"

        bypage_list = []
        for page in scrape_dict.get('bypage', []):
            bypage_list.append(get_bypage_value_safely(page))
        return bypage_list
    ...
    async def test_contact_extraction(self):
        ...
        for test_file in test_file_list:
            with open(f"{test_folder_path}/{test_file}.json", "r") as f:
                ...
                blocks_list  = await self.get_bypage_list_safely(scrape_dict)
