...
class contact_generation_pipeline:
    def __init__(self, environment='aws'):
        ...
        self.misleading_domains = ['gmail', 'yahoo', 'hotmail', 'aol', 'outlook', 'icloud', 'protonmail', 'linkedin', 'facebook', 'twitter', 
                                   'youtube', 'instagram', 'www', 'vimeo', 'glassdoor', 'tiktok', 'x', 'podbean', 'apple', 
...
#*################################################################ 
    ...
    async def apply_upsert(self, base_row, new_row, append_or_replace="REPLACE"):
        ...
        for key in base_row.keys():
            ...
            # Continue if the key is not in the new row and there is some worthwhile data in the new row
            if key not in new_row.keys() or isnull_or_empty(new_row.get(key)) or new_row.get(key) == base_row.get(key) or new_row.get(key) == [] or new_row.get(key) == {}:
                # Proceed with whatever is on the base row
                row_to_return[key] = base_row.get(key)
                continue
            
            if isnull_or_empty(base_row.get(key)) and not isnull_or_empty(new_row.get(key)):
            ...
            if isinstance(base_row.get(key), list):
...
#*################################################################
    ...
    async def async_scrape_single_url(self, url):
        ...
        async def extract_same_domain_urls(page, base_url):
            ...
            logger.error(f"Found {len(anchor_elements)} anchor elements")
        ...
        try:
            async with async_playwright() as p:
                ...
                try:
                    await page.goto(url, timeout=30000)  # Set a timeout for page navigation
                    await page.wait_for_selector('body', timeout=30000)  # Set a timeout for waiting for body
                
                except PlaywrightTimeoutError as e:
                    return_dict["error_message"] = f"Timeout error for URL |{url}|: {str(e)}"
                    return_dict["success"] = False
                    await browser.close()
                    return return_dict
        ...
        #     "NC":"\033[0m'"} # No Colo"}
        # "RED":"\033[1;31m",
            logger.error(f"\033[1;31mAfter {datetime.now() - scrape_start_time} seconds, the scrape failed for {url} with error: {str(e)}\033[0m")
            return return_dict
        
        except Exception as e:
            logger.error(f"An unexpected error occurred during the Playwright scraping: {str(e)}")
            return_dict["error_message"] = f"An error occurred for URL |{url}|: {str(e)}"
            return_dict["success"] = False
            logger.error(f"\033[1;31mAfter {datetime.now() - scrape_start_time} seconds, the scrape failed for {url} with error: {str(e)}\033[0m")
            return return_dict
        ...
        # if override_to_update_db:
        #     await self.upsert_data([return_dict], table_name='targetlist', schema="public", column_name_of_current_record_key='uniquebuskey', accommodate_system_fields=False, new_processing_status=None)
        return return_dict 
...
#!################################################################
    ...
    async def get_next_batch_of_targets(self, batch_size=20, start_processing_status='NEW', optional_ubuskey=None):
    ...
    async def search_for_executives(self, scrape_dict ):
        ...
        query = f"{name} {website_domain} executives -linkedin -facebook -instagram"
        scrape_dict['google_search_String'] = query
        logger.error(f"Searching for executives with query: {query}")
    ...
    async def scrape_seed_urls(self, scrape_dict, next_processing_status='SCRAPE1'):
        ...
        if current_urls is None:
            current_urls = []
        scrape_dict['urls'] = current_urls.append(scrape_dict.get('website_address', ''))
        ...
        gurls = []
        seed_scrape_results = []
        for url in scrape_dict.get('urls', []):   
        ...
        # results = run_multiprocessing(scrape_dict.get('urls', []))
        # seed_scrape_results = results
                    
                    seed_scrape_results.append( self.async_scrape_single_url(url=url))
        results = await asyncio.gather(*seed_scrape_results)
        ...
        if len(results) > 0:
            ...
            for result in results:
                ...
                single_result['updatedon'] = datetime.now()
    ...
    async def scrape_extended_urls(self, scrape_dict, max_urls_per_client=1):
        ...
        if struct_data_list_of_strings:
            for struct in struct_data_list_of_strings:
                if isinstance(struct, str):
                    struct_dict = get_dict_from_mess(struct)
        ...
        # Break the URLs into groups of 10 and process each group
        batch_size = 25
        results = []
        ...
        if len(results) > 1:
            ...
            if scrape_dict.get('bypage', []) is None or len(scrape_dict.get('bypage', [])) == 0:
                scrape_dict['bypage'] = url_page
            else:
                scrape_dict['bypage'].append(url_page)
    ...
    async def get_framework_from_gemini(self, scrape_dict):
        ...
        for page in scrape_dict.get('bypage', []):
            ...
            if isinstance(page, str):
                clean_str = get_dict_from_mess(page)
        ...
        for scrape_dict in scrape_dicts:
            # try:
                ...
                if start_processing_status == 'NEW':
                    ...
                    emails = self.extract_emails(f"{scrape_dict.get('bypage', '')}")
                    if emails and len(emails) > 0:
                        try: 
                            ...
                            continue
                    logger.log(f"\n\033[1;36m** Email extraction complete for {scrape_dict.get('primary_business_name', 'NO NAME')} with uniquebuskey {scrape_dict.get('uniquebuskey', 'NOKEY')}\033[0m")

                    
                    #! 4 Extract Names
                    logger.log(f"\n\033[1;36m** Starting name extraction for {scrape_dict.get('primary_business_name', 'NO NAME')} with uniquebuskey {scrape_dict.get('uniquebuskey', 'NOKEY')}\033[0m" )
                    ext_names_dict = asyncio.run(self.extract_names(f"{scrape_dict.get('bypage', '')}"))
                    scrape_dict['names'] = ext_names_dict.get('validnames', [])
                    logger.log(f"\n\033[1;36m** Name extraction complete for {scrape_dict.get('primary_business_name', 'NO NAME')} with uniquebuskey {scrape_dict.get('uniquebuskey', 'NOKEY')}\033[0m")    

                    #! 5 Get the framework from Gemini
                    logger.log(f"\n\033[1;36m** Starting framework extraction for {scrape_dict.get('primary_business_name', 'NO NAME')} with uniquebuskey {scrape_dict.get('uniquebuskey', 'NOKEY')}\033[0m")
                    scrape_dict['gemini_initial'] = asyncio.run(self.get_framework_from_gemini(scrape_dict))
                    logger.log(f"\n\033[1;36m** Framework extraction complete for {scrape_dict.get('primary_business_name', 'NO NAME')} with uniquebuskey {scrape_dict.get('uniquebuskey', 'NOKEY')}\033[0m"    )
                    
                    #! 6 Get the final contacts from Gemini
                    scrape_dict['contacts'] = asyncio.run(self.get_final_contacts_from_gemini(scrape_dict['gemini_initial'], scrape_dict))
                    
                    # Update the processing status
                    scrape_dict['processingstatus'] = next_processing_status
                    
                    try:
                        logger.log(f"\n\033[1;36m** Starting final contact extraction for {scrape_dict.get('primary_business_name', 'NO NAME')} with uniquebuskey {scrape_dict.get('uniquebuskey', 'NOKEY')}\033[0m")
                        contact_dict_json = {}
                        if isinstance(scrape_dict.get('contacts', None), list):
                            iterable_list = scrape_dict.get('contacts', [])
                        elif isinstance(scrape_dict.get('contacts', None), dict):
                            iterable_list = scrape_dict.get('contacts', {}).get('response', [])
                        else:
                            continue
                        
                        # Convert the list of contact dictionaries to a single dictionary
                        # with the name as the key and set that to the value of scraper_dict['contacts']
                        # that single dictionary will be merged APPEND with any current contact data.
                        for contact in iterable_list:
                            if not isinstance(contact, dict):
                                continue
                            con_dict = {}
                            con_dict[contact.get('name', 'NO-NAME')] = contact
                            contact_dict_json.update(con_dict)
                        scrape_dict['contacts'] = contact_dict_json
                        logger.log(f"\n\033[1;36m** Final contact extraction complete for {scrape_dict.get('primary_business_name', 'NO NAME')} with uniquebuskey {scrape_dict.get('uniquebuskey', 'NOKEY')}\033[0m")   
                        
                        # Update the database with the new scrape results
                        logger.log(f"\n\033[1;36m** Starting upsert for {scrape_dict.get('primary_business_name', 'NO NAME')} with uniquebuskey {scrape_dict.get('uniquebuskey', 'NOKEY')}\033[0m") 
                        ...
                        logger.error(f"Error converting contacts to dictionary: {str(e)}")
                        with open(f"scrapes/{scrape_dict.get('primary_business_name', 'NO_NAME')}_contacts.json", "w") as f:
                            f.write(json.dumps(scrape_dict), indent=4)
                        continue
            ...
            # except Exception as e:
            #     logger.error(f"Error in pipeline: {str(e)} for {scrape_dict.get('primary_business_name', 'NO NAME')} with uniquebuskey {scrape_dict.get('uniquebuskey','')}")
            #     continue
        return True
...
if __name__ == "__main__":
    ...
    results = asyncio.run(gen_contact.execute_listgen_pipeline(batch_size=10, max_urls_per_client=75, start_processing_status='NEW', next_processing_status='SCRAPE1'),) 
