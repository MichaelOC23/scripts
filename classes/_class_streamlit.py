...
BASE_FLASK_URL = base_url="http://127.0.0.1:4003/"
    ...
    async def upsert_data(self, data_items, table_name=None):
        ...
        # try:
            for item in data_items:
                test = await conn.upsert_append_data(item, table_name)
                async with conn.transaction():
...
class transcription_library_crud():
    def __init__(self):
        self.transcription_table = "textlibrary"
    # Database connection
    def get_db_connection(self):
        db_url = os.environ.get('POSTGRES_AWS_PRODUCT_DB_CONN_STRING', 'POSTGRES: COULD NOT GET CONNECTION STRING')
        engine = create_engine(db_url)
        return engine.connect()

    # Load data from the database
    def load_data(self, conn):
        query = f"SELECT * FROM {self.transcription_table} where iscurrent = true"
        return pd.read_sql(query, conn)

    # Insert a new record into the database
    def insert_data(self, conn, data):
        data.to_sql(self.transcription_table, conn, schema='mytech', if_exists='append', index=False)

    # Update an existing record in the database
    def update_data(self, conn, data):
        data.to_sql(self.transcription_table, conn, schema='mytech', if_exists='replace', index=False)

    # Delete a record from the database
    def delete_data(self,  conn, data):
        query = f"DELETE FROM {self.transcription_table} WHERE id = {data['id'].iloc[0]}"
        conn.execute(query)
...
class deepgram_audio_transcription():
    ...
    def call_local_api(self, flask_path, params=None):
        url = self.base_flask_url + flask_path
        response = requests.get(url, params=params)
        return response.text
    
    def get_audioinfo(self):
        audio_info_str = self.call_local_api("getaudioinfo")
        return json.loads(audio_info_str)
    ...
    def get_audio_device_dict(self):
        import pyaudio
        ...
        for i in range(0, num_devices):
            info = p.get_device_info_by_index(i)
            device_dict[i]=f"#{i}: {info['name']} ({info['maxInputChannels']}in)"
        return device_dict
    ...
    async def load_json_to_table(self, connection_string, table_name, json_file_path):
        ...
        try:
            ...
            INSERT INTO {table_name} (
                ...
                Legal_Name,
                Main_Office_Location  NULL, 
                Main_Office_Telephone_Number  NULL,
                SEC_Executive  NULL,
...
class office_365_data():
    ...
    def __init__(self):
        ...
        self.jbi_site_id = os.environ.get('JUSTBUILDIT_SHAREPOINT_SITE_ID', 'No Key or Connection String found')
        self.drive_id = os.environ.get('JUSTBUILDIT_SHAREPOINT_DRIVE_ID', 'No Key or Connection String found')
        self.item_path = '/sites/Communify/08_DataModel/JustBuildIt/Release.2024.06.05.xlsx'
    ...
    def get_drives_for_site(self, site_id=None):
    ...
    def get_all_sharepoint_site_ids(self):
        ...
        Site_List = []  
        for item in site_response.json()['value']:
            Site_List.append({item['id']: item['name']})
        return Site_List
    
    def get_site_id(self, site_url=None):
        if site_url is None or site_url == '':
            site_url = 'https://graph.microsoft.com/v1.0/sites?search=*'
        headers = {'Authorization': f'Bearer {self.access_token}'}
        site_response = requests.get(f'https://graph.microsoft.com/v1.0/sites/{site_url}', headers=headers)
        site_data = site_response.json()
        site_id = site_data['id']
        print(f'Site ID: {site_id}')
    
    def get_o365_data(self, entity, endpoint):
        
        headers = {'Authorization': f'Bearer {self.access_token}'}
        response = requests.get(f"{self.GRAPH_ENDPOINT}/{self.end_points.get(entity, {}).get(endpoint, '')}", headers=headers)
        if response.status_code == 200:
    ...
    def get_calendar_events(self):
        return self.get_o365_data('Calendar', 'Get')
    
    def get_emails(self):
        return self.get_o365_data('Email', 'Get')
    ...
    def is_authenticated_to_o365(self):
        ...
        if os.path.exists(f"{home}/.jbi/at.json"):
            with open(f"{home}/.jbi/at.json", 'r') as f:
                at = json.load(f)
                self.access_token = at.get('access_token', None)
...
# # asyncio.run(prospect_list.load_json_to_table(prospect_list.connection_string, "prospect", "/Users/michasmi/Downloads/ia050324-exempt.json"))
