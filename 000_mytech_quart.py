...
#########################################
OFFICE365_BACKGROUND_PORT = os.environ.get('MYTECH_QUART_PORT', 4008)
...
CLIENT_ID = "677433d4-b486-43a5-a8d6-26653ae69bb7"
...
MSFT_TOKEN_JSON = os.environ.get('MSFT_TOKEN_JSON', '')
...
def calculate_expiration(expires_in_seconds):
...
async def auth_redirect():
    ...
    if response.status_code == 200:    
        ...
        if token_json.get('expires_in', 0) > 2700:
            expiration_time_str = calculate_expiration(2700)
        else:
            expiration_time_str = calculate_expiration(token_json['expires_in'])
...
async def create_task(task_request=None):
    global access_token
    data = await request.get_json()
    ...
    task_details = create_todo_task_with_user_request(user_request, access_token)
...
def create_todo_task_with_user_request(user_request, access_token):
    """
...
def parse_response(response_text):
    ...
    If the first attempt fails, a second attempt is made by discarding text outside the JSON object.

    :param response_text: The raw response text to parse.
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=OFFICE365_BACKGROUND_PORT, debug=False)
