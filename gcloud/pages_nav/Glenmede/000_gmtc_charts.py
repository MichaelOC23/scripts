...
import numpy as np
import csv
print(f"Time to load imports: {time.time() - time_start}")

import streamlit as st
from st_aggrid import AgGrid, GridOptionsBuilder, GridUpdateMode, DataReturnMode
print(f"Time to load streamlit: {time.time() - time_start}")



print(f"Time to load custom classes: {time.time() - time_start}")
...
if __name__ == '__main__':
    ...
    for key in gmtc.dataframes.keys():
        # Add accountnum column to all relevant dataframes
        if 'portnum' in gmtc.dataframes[key].columns:
            gmtc.dataframes[key]['accountnum'] = gmtc.dataframes[key]['portnum'].str.replace(r'\.\d+$', '', regex=True)
    ...
    holdings_df['secid'] = holdings_df['ticker']

    # Step 2: Where 'ticker' is empty/blank/null, set 'secid' to 'cusip'
    holdings_df['secid'] = np.where(holdings_df['secid'].isnull() | holdings_df['secid'].str.strip().eq(''), holdings_df['cusip'], holdings_df['secid'])

    # Step 3: Set any remaining blanks in 'secid' to 'USD'
    holdings_df['secid'] = holdings_df['secid'].replace('', 'USD')
    holdings_df['secid'] = holdings_df['secid'].fillna('USD')

    
    accounts_by_asset_class = holdings_df.groupby(['accountnum', 'assetclass'])['marketvalue'].sum().reset_index()
    
    # Accounts by Sub Asset Class
    accounts_by_sub_asset_class = gmtc.dataframes.get('account_holdings.csv').groupby(['accountnum', 'assetclass'])['marketvalue'].sum().reset_index()
    st.caption(accounts_by_sub_asset_class.dtypes)
    
    #Models
    asset_class_models = gmtc.dataframes.get('account_models.csv')[gmtc.dataframes.get('account_models.csv')['policyclasstype'] == 'AssetClass']
    sub_asset_class_models = gmtc.dataframes.get('account_models.csv')[gmtc.dataframes.get('account_models.csv')['policyclasstype'] == 'AssetSubClass']
    ...
    for key in gmtc.dataframes.keys():
        # try:
            ...
            with st.expander(name, expanded=False):
                grid.display_grid( gmtc.dataframes[key])
