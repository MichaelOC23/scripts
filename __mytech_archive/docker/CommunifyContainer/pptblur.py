...
class powerpoint_tools():
    ...
    def blur_all_images_in_pptx(self, file_path):
        ...
        for slide in presentation.slides:
            ...
            output_path = f'/Users/michasmi/Downloads/blur/{slide.slide_id}_blurred.pptx'
            ...
            for shape in slide.shapes:
                ...
                if shape.shape_type == MSO_SHAPE_TYPE.PICTURE and shape.shape_id not in processed_shape_ids:
                    ...
                    try:
                        image = shape.image
                        ht, wd = shape.image.size
                        if ht < 500 or wd < 500:
                            continue
                        if "object" in shape.name.lower():
                            continue
                        image_bytes = image.blob
                        blurred_image_bytes = apply_blur_to_image(image_bytes, 20)
                        
                        unique_shape_id = shape._element.get('id')
                        
                        # Save the blurred image to a temporary file
                        temp_image_path = f'/Users/michasmi/Downloads/blur/images/{slide.slide_id}_{shape.name}_temp_image.png'
