...
def get(retries):
    ...
    indus_list = []
    for tick in Tickers:
        indus = (f"{tick.get('Id', '')}  | Portfolio Percent {tick.get(f'ValuePercent', '')}")
        indus_list.append(indus)
    

    message_content = f"My porfolio contains the following percentage breakdown of industry sectors: {" ".join(indus_list)}. \n  Please provide your commentary"
