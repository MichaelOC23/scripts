...
class NatsClass:
    ...
    def run_nats_request(self, subject, payload):
        # Synchronous wrapper for the asynchronous function
        def run(loop, nc, subject, payload):
            result = {'data': None}

            async def async_request():
                await nc.connect(self.base_url, loop=loop)
                try:
                    response = await nc.request(subject, payload.encode(), timeout=1)
                    result['data'] = response.data.decode()
                except Exception as e:
                    result['data'] = "Request timed out"
                finally:
                    await nc.close()
        ...
        nc = NATS()
        result = run(loop, nc, subject, payload)
        return result
