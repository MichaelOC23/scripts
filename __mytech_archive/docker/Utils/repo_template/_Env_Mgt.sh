...
DNAME="SETAVALUE"  #<<<<<<<<< SET THE VALUE
DPATH="${HOME}/code/mytech/docker/$DNAME"
DPORT=4001


# Test if DNAME is = SETAVALUE, if it is exit and print error message
if [ "$DNAME" = "SETAVALUE" ]; then
    echo -e "\033[1;31mPlease set the value of DNAME at the top of the script\033[0m"
    exit 1
fi
