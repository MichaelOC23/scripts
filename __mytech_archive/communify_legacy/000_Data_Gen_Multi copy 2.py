...
# from Class_Load_Data import CSVLoader

# csv = CSVLoader("dbname=platform user=postgres password=test host=localhost port=5432")
...
def log_it(message, print_it=True, color='black'):
    ...
    with open(os.path.join(LOGS, 'data_gen_log.txt'), 'a') as f:
        f.write(message)
        print(message)
def multi_process_csv_chunk(chunk, key_column):
    chunk_dict = {}
    ...
    return chunk_dict
def csv_to_dict_merge_multiprocessing(csv_file_path, key_column, chunk_size=10000):
    merged_dict = {}
    ...
    with Pool() as pool:
        ...
        results = pool.starmap(multi_process_csv_chunk, [(chunk, key_column) for chunk in chunks])
    ...
    return merged_dict
def csv_to_dict(csv_file_path, key_column='AttributeName'):
    result_dict = {}
...
def load_csvs_to_postgres():
    ...
    # load_tasks.append(csv.load_data(Positions_To_Load, "Position"))
...
#!######    Price Hisotry:  Get and Create CSV    ######
# Ony used as needed for development

ticker_split_str = "NVDA"

def get_price_history_for_stock(stock_to_price):
...
get_price_history_for_stock(ticker_split_str)
...
def get_listed_tickers():
    def process_titles(data):
        ...
        return data
    archive_source_file(sec_ticker_dictionary_path)
...
def get_sic_table():
    ...
    archive_source_file(sic_code_file_path)
    # Save the new file to the data folder
...
def merge_chase_data(row, EmptyPositionEntity):
...
############################################
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        future_get_listed_tickers = executor.submit(get_listed_tickers)
    ...
    SecurityMasterEnitty = csv_to_dict(securitymaster_entity_path)
...
#!  ------>  Process the Position Data  <------  !#
    ...
    PositionEntityDefinition = csv_to_dict(position_entity_path)
    ...
    for row in positions_chase_dict:
        position = merge_chase_data(row, EmptyPositionEntity)
...
def load_file_to_postgres(csv_file_path, table_name):
    import psycopg2
