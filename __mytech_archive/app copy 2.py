...
from _class_firebase_insert_dict import firebase_insert_dictionary
fid = firebase_insert_dictionary()
...
if True:
    ...
    if 'open_note_document_id' not in st.session_state: st.session_state['open_note_document_id'] = ''
    if 'note_date_value' not in st.session_state: st.session_state['note_date_value'] = None
    ...
    if 'open_note_content' not in st.session_state: st.session_state['open_note_content'] = ''
    ...
    if 'prior_note_radio_987'not in st.session_state: st.session_state['prior_note_radio_987'] = 'NO_VALUE'

    # Editor  Session State Variables
    if 'note_editor_disabled' not in st.session_state: st.session_state['note_editor_disabled'] = False
    if 'ace_themes' not in st.session_state: st.session_state['ace_themes'] =  [ "chrome", "clouds", "iplastic", "solarized_light", 
                                                                                "pastel_on_dark", "solarized_dark", "cobalt", 
                                                                                "tomorrow_night_blue", "twilight"]
...
if True: 
    ...
    def display_task_header():
        ...
        if create_task.button("", key="topicbutton", use_container_width=True, type="primary", icon=':material/view_object_track:'): create_topic_modal()
...
if True:
    ...
    #! Creating, updating and getting topics
    @handle_async_operation
    ...
    @handle_async_operation
    async def assemble_topic_data():
        
        if not st.session_state['topic'] or st.session_state['topic'] == '':
            st.error("Cannot load topic. No Topic set.")
            return
        
        # Populate topic_dict
        topic_document_id = st.session_state['topics_by_name_dict'][st.session_state['topic']]

        # Starting Topic Detail Session State Variables
        st.session_state['topic_dict'] = st.session_state['topics_dict'].get(topic_document_id, {})

        # Starting Note Data Detail Session State Variables
        st.session_state['note_dicts_dict_for_open_topic'] = await fire_db.get_recent_documents_from_collection(
                    collection_name=st.session_state['collection'], 
                    filters={"type": "note", "topic_document_id": topic_document_id}
                    )
        
        for key, value in st.session_state['note_dicts_dict_for_open_topic'].items():
            if isinstance(value, datetime):
                st.session_state['note_dicts_dict_for_open_topic'][key] = fire_db.datetime_to_streamlit_date(value)
                
    
    @handle_async_operation
    async def assemble_note_data():

        topic_document_id = st.session_state['topics_by_name_dict'][st.session_state['topic']]

        # Check of there are some notes for the topic and if no note is already selected, select the first note.
        note_document_id = st.session_state['open_note_document_id']
        if not note_document_id or note_document_id == '':
            if st.session_state['note_dicts_dict_for_open_topic'] and len(st.session_state['note_dicts_dict_for_open_topic'].items())>0:
                # Select the first existing note
                note_document_id = list(st.session_state['note_dicts_dict_for_open_topic'].keys())[0]
            else:
                # Create a new note and select it as the first note
                note_document_id = create_new_note_for_topic(topic_document_id)
                if note_document_id:
                    st.rerun()
        
        # set the values for the first/new note
        if st.session_state['open_note_dict']   == {} and note_document_id and  note_document_id != '':
            st.session_state['open_note_dict'].update(st.session_state['note_dicts_dict_for_open_topic'].get(note_document_id))
            st.session_state['open_note_content'] = st.session_state['open_note_dict'].get('note_md', '')
    ...
    def clear_topic():
        async_save_note(st.session_state['temp_note_content'])  # Call async save function
    ...
    def display_topic():
        ...
        def format_note_radio(value):
            try: return st.session_state['note_dicts_dict_for_open_topic'][value].get('note_title', 'No title')
        ...
        logo_url = topic_dict.get('logo', '')
        
        if not logo_url or logo_url == '': 
            logo_sidebar_sh_col.image(st.session_state['default_logo_url'] ,use_container_width=True)
        else:
            formatted_image = format_image( logo_url)
            logo_sidebar_sh_col.image(formatted_image,use_container_width=True)
        
        name_sh_col.subheader( topic_dict.get('topic', 'No Topic Details . . .'))
        name_sh_col.markdown(f"**Synonyms:** {', '.join(topic_dict.get('alt_topics', ['No synonyms']))}")
        # with syn_sh_col:
        #     fd['alt_topics'] = st_tags(label='Synonyms', text='Press enter to add more', value=topic_dict.get('alt_topics', []), maxtags = 10, key='focus_tags_syn')
        ...
        if st.session_state['note_radio_987'] != st.session_state['prior_note_radio_987'] and st.session_state['note_radio_987'] != '':
                st.session_state['prior_note_radio_987'] = st.session_state['note_radio_987']
                st.rerun()
        ...
        nbtn4.button(
            ...
            kwargs={'topic_document_id': topic_dict.get('document_id')},
            type='secondary')
    ...
    #! ONE_TIME LOAD OF DEFAULT TOPICS
    @handle_async_operation
...
if True:
    ...
    def create_empty_note(topic_document_id, topic_title, note_document_id):
        ...
        note_number = len(list(st.session_state['note_dicts_dict_for_open_topic'].keys())) + 1
        
        note_dict = {"note_title" : f"{topic_title} Note #{note_number}",
    ...
    def create_new_note_for_topic(topic_document_id):
        new_note_document_id = f"{uuid.uuid4()}"
        empty_note_dict = create_empty_note(topic_document_id=topic_document_id, 
                                            topic_title=f"{st.session_state['topic']}", 
                                            note_document_id=new_note_document_id)
        
        st.session_state['note_dicts_dict_for_open_topic']['new_note_document_id'] = empty_note_dict
        
        fire_db.sync_insert_dictionary( collection_path=st.session_state['collection'], 
                                       data_dictionary=empty_note_dict, 
                                       document_id=new_note_document_id)
        
        clear_note()
        st.session_state['open_note_document_id'] = new_note_document_id
        assemble_note_data()
        st.rerun
    ...
    @handle_async_operation
    async def async_save_note(content):
        try: 
            ...
            if open_note_document_id:
                ...
                st.session_state["open_note_content"] = content
            return True
        except Exception as e:
            ...
            return False
    def sync_save_note(content, title, document_id, topic_document_id):
        
        if not document_id:
            st.error('No document id to save note.')
            st.stop()
            ...
            fid.execute_code(
                ...
                dict_to_insert=data_dict
                )

    
    #! DISPLAYING NOTES
    def clear_note():
        st.session_state['open_note_document_id'] = ''
        st.session_state['open_note_dict'] = {}
        st.session_state['open_note_content'] = ''
        st.session_state['temp_note_content'] = ''
        st.session_state['note_editor_disabled'] = False    
    
    def set_display_note_callback(session_state_key):
        # Get the passed in 'note_document_id'
        open_note_document_id = st.session_state.get(session_state_key)
        # Clear the note context and set the new value of open_note_document_id
        clear_note()
        
        # Use the new note_document_id value to refresh data and render the screen
        st.session_state['open_note_document_id'] = open_note_document_id
        assemble_note_data()
        
    
    def display_open_note():
        note_document_id = st.session_state.get('open_note_dict', {}).get('document_id', '')
        topic_document_id = st.session_state.get('open_note_dict', {}).get('topic_document_id', '')
        with st.container(key=f'container_{note_document_id}'):
            ...
            size_box, wrap_box = nc1.columns([3,1])
            
            # Note Title
            updated_title = nc1.text_input("Title / Date", 
                            value=st.session_state.get('open_note_dict', {}).get('note_title', ''), 
                            key=f"note_title{note_document_id}")
            ...
            # Word Wrap
            wrap = wrap_box.checkbox("Wrap enabled", value=True, key=f"wrapcheck{note_document_id}")
            
            # Size / Height
            note_height = size_box.radio('Note Area', options=['Small','Medium', 'Large'],key=f"note_height{note_document_id}",horizontal=True, label_visibility='collapsed') 
            note_height_dict = {"Small": 5, "Medium": 20, "Large": 40 }
            
            ss_key = st.session_state.get('open_note_dict', {}).get('document_id', '')
            if not ss_key or ss_key == '':
                st.error('Note does not have a document id')
                st.stop()
            else: 
                temp_key = f'temp_{ss_key}'
                real_ss_key = f'real_{ss_key}'
            
            st.session_state[temp_key] = st_ace(
                    value=st.session_state["open_note_content"],
            ...
            if st.session_state[temp_key] and st.session_state[temp_key] != st.session_state[real_ss_key]:
                save_status_message.caption("*Saving changes...*")
                st.session_state[real_ss_key] = st.session_state[st.session_state[temp_key]]
                sync_save_note(content=st.session_state[st.session_state[temp_key]], title=updated_title, document_id=ss_key, topic_document_id=topic_document_id)
                # async_save_note(st.session_state['temp_note_content'])  # Call async save function
                save_status_message.caption ("Saved")
                
            # Display the saved content
            st.subheader("Saved Content:")
            st.markdown(st.session_state[real_ss_key])
...
if True:
    ...
    def display_tasks_or_one_topic_and_notes():    
        if st.session_state['topic'] and st.session_state['topic'] != '':
            ...
            # Tasks as Grid
            if st.session_state['view_type'] == 'Grid': display_tasks_as_grid()
            # Tasks as Kanban
            else: display_tasks_kanban()
    ...
    pages = {
        "Efficiency": [
            st.Page(display_tasks_or_one_topic_and_notes, title="Task Dashboard", url_path="Task_Dashboard"),
