...
import uuid, chardet, os, io, json, time, csv, shutil, matplotlib, sys, re, math, random,  asyncio, httpx
...
class TextExtractor:
    ...
    async def init_extractor(self):
        ...
        for file_path in tqdm_asyncio(pdf_files_to_process, desc="Processing PDF Files"):
            pdf_dict = await self.pdf_to_text(file_path)
            print (f"{pdf_dict.get('success', False)}: Processing {file_path}")
    ...
    def url_encode_file_name(self, file_name):
    ...
    async def pdf_to_text(self, file_path, pdf_dict = {'success': False}):
        

        # Add the file path to the PDF Dictionary that will conatain all the results
        pdf_dict['file_path'] = file_path
        ...
        image_folder_relative_path = await self.sanitize_for_url(f"images_{file_path}_{datetime.now().strftime("%H%M%S")}")
        ...
        # Convert the PDF into a list of images (each page = 1 image)
        images = convert_from_path(file_path)
        
        #Remove any problematic characters from the file name
        file_name = os.path.basename(file_path)
        pdf_dict['file_title'] = file_name
        file_name = self.remove_special_characters(file_name)
        file_name = self.url_encode_file_name(file_name)

        pdf_dict['file_name'] = file_name
        
        
        # Extract the text from each image and attempt to clean it.
        image_dicts=[]
        image_number = 0
        client = gc_storage.Client()  # Initialize the Google Cloud Storage client
        bucket_name = 'toolstorage'  # Replace with your Firebase Storage bucket name
        bucket = client.bucket(bucket_name)
        for image in tqdm_asyncio(images, desc="Processing PDF Pages (images)"):
            image_dict = {}
            # Properly name and number the image
            image_number += 1
            image_dict['image_number'] = image_number
            image_title = f"{file_name}_Page_{image_number}.png"
            image_dict['image_title'] = image_title
            
            # Save the image
            image_path = f"{page_images_absolute_path}/{image_title}"
            image_dict['path'] = image_path
            image.save(image_path, 'PNG')
            
            # Extract text and append to image_dict
            image_dict = await self.image_to_text(image_path, image_dict=image_dict)
            
            try:
                blob = bucket.blob(f"documents/pdfpageimages/{image_title}")
                blob.upload_from_filename(image_path)
                blob.make_public()  # Make the uploaded image publicly accessible
                image_url = blob.public_url
                image_dict['image_url'] = image_url
            except Exception as e:
                print(f"Failed to upload image {image_title}: {e}")
                continue
            prompt = await self.create_prompt_to_clean_ocr_text()
            # Create the markdown for the image
            ai_response = await self.fetch_ollama_response(prompt=prompt, data=image_dict['clean_text'])
            image_dict['markdown'] = f"{ai_response}"
            image_dicts.append(image_dict)
        
        # No pages were found, return error
        if not image_dicts or len(image_dicts) == 0: return pdf_dict
        
        pdf_dict['page_count'] = image_number
        
        #Get the first 1 or 2 pages of text and use llm to create a title
        starting_text =  image_dicts[0].get('clean_text', '')
        if len(image_dicts) > 1: 
            starting_text = f"{starting_text}\n{image_dicts[1].get('clean_text', '')}"   
        
        # Submit the first part of the text to Gemini to get the title of the document
        title = await self.try_and_get_document_title(starting_text)
        pdf_dict['title'] = title
        
        # Upload the pdf to get a url    
        pdf_url = await fire.upload_file(file_path, destination_blob_name=f"documents/pdfs/{file_name}")
        pdf_dict['pdf_url'] = pdf_url
        
        # Attach the pages to the pdf and insert the complete pdf_dictionary
        pdf_dict['pages'] = image_dicts
        pdf_dict['success']=True
        await fire.insert_dictionary('document-library', pdf_dict, f"{file_name}")
    ...
    async def fetch_ollama_response(self, prompt=None, data=None, embeddings = None, model=None, timeout=15, return_data_on_error=False):        
        KeepTrying = True
        TryCount = 1
        return_value_on_error = None
        if return_data_on_error: return_value_on_error = data
        if not model: model = os.environ.get('OLLAMA_BEST_LANGUAGE', 'phi3.5:latest')
        if not prompt: return return_value_on_error
        if data: prompt = f"{prompt}\n{data}"
        if embeddings: prompt = f"{prompt}\n\n\n---  \nPlease incorporate and prioritize the following information when generating your response:  \n{embeddings}"
        ...
        while KeepTrying and TryCount <= 3:
            ...
            try:
                ...
                tqdm_asyncio.write(f"TQDM Error in fetch_ollama_response response: {e}")

                time.sleep(1)
                if TryCount > 2:
                    KeepTrying = False
                    tqdm_asyncio.write(f"Error: Failed to get Ollama response after {TryCount} tries.")
                    pass
                    ...
                    tqdm_asyncio.write(f"Warning: Retrying Ollama request {TryCount}...")
                    pass
                    time.sleep(1)
        ...
        try:
            ...
            value_to_return = response_text
            return value_to_return

        except Exception as e:
            tqdm_asyncio.write("Could not process the response from Ollama")
            if response:
    ...
    async def try_to_json(self, input_value):
        try:
            sanitized = re.sub(r'"""(.*?)"""', lambda m: json.dumps(m.group(1)), input_value, flags=re.DOTALL)
    
            # Ensure proper escaping of newline characters
            sanitized = sanitized.replace("\\n", "--safenewline--")
            sanitized = sanitized.replace("\n", "\\n")
            sanitized = sanitized.replace("--safenewline--", "\\n")
    ...
    async def create_prompt_to_clean_ocr_text(self):    
        ...
        instructions = """
        Transform the following OCR-extracted text into clean, readable MARKDOWN. 
        Return the MARKDOWN without any additional commentary. The text is part of a longer document that was split by page,
        ...
        Your response is being systematically integrated. Only return the MARKDOWN as text. No other commentary or data.
        """
        return instructions
    ...
    async def try_and_get_document_title(self, some_text, LLM='Ollama'):
        # pass
        # # self.logger.log(f"CurFunct11: {inspect.currentframe().f_code.co_name}", force_print=True, color="CYAN")
        ...
        try: 
            ...
            if LLM == 'Ollama':
                response = await self.fetch_ollama_response(prompt)
            else:
    ...
    async def image_to_text(self, file, image_dict = {'success': False}):
        ...
        file_type = os.path.splitext(file)[1] 
        image_dict['file_type'] = file_type
        
        # Invalid Image Type
        if file_type not in self.image_types: return image_dict
        ...
        try:
            ...
            image_dict['raw_text'] = image_raw_text
            
            # Remove useless Lines
            clean_text = await self.semi_improve_image_ocr(image_raw_text)
            image_dict['clean_text'] = clean_text
            
        except Exception as e:
            image_dict['error'] = f"{e}"
            return image_dict
        
        image_dict['success'] = True
        return image_dict
...
class FirestoreStorage:
    def __init__(self, default_collection='original_json_transcription', default_bucket='toolstorage'):
        self.db = firestore.Client(project='toolsexplorationfirebase')
...
if __name__ == "__main__":
    ...
    test_folder = "/Users/michasmi/Obsidian/Document-Library/test"
