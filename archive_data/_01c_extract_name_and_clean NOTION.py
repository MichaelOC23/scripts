...
from _class_notion_api import NotionDBManager

def process_image_pool(image_param):
...
class TextExtractor:
    ...
    async def pdf_to_text(self, file_path, page_images_relative_path, page_images_absolute_path):
        ...
        for i, image in enumerate(images):
            image_path = os.path.join(page_images_absolute_path, f"page_{i + 1}.png")
            image.save(image_path, 'PNG')
            images_param.append(image_path)
        
        num_processes = min((os.cpu_count() - 4), len(images_param))
        
        self.logger.log(f"PDF converted, beginning image processing for '{file_path}'")

        # Ensure synchronous execution context to use `ProcessPoolExecutor` with asyncio
        loop = asyncio.get_event_loop()
        # results = []
        # for image in images_param:
        #     results.append(process_image_pool(image))
        
        self.logger.log(f"Image processing complete for '{file_path}'")
        
        with ProcessPoolExecutor(max_workers=num_processes) as executor:
            futures = [
                loop.run_in_executor(executor, process_image_pool, param) 
                for param in images_param
            ]
            results = await asyncio.gather(*futures)
        
        self.logger.log(f"Completed image processing for '{file_path}'")
        ...
        elif len(results) > 1: starting_text = f"{results[0][0]}\n{results[1][0]}"   
        ...
        for result in results:
            ...
            image_path = result[1]
            image_text = result[0]
            image_url = await self.upload_to_azure(image_path)
            notion_image_text_list.append([page_count, image_path, image_text, image_url])
            

            
        
        parent_id = self.notion.get_first_id_search("[Client Imports]")
        new_db_id = self.notion.create_file_import_db(possible_title, notion_image_text_list, "📁", parent_id)
                
            
        # for i, page in enumerate(results):
        #     all_text = f'''{all_text}\n___ \n<span style="color:#6986B0; font-size:20px; font-weight:bold;">Page {i + 1}</span> \n\n{sentence}\n'''

        return "", possible_title, page_count
    ...
    async def upload_to_azure(self, file_path):
        try:
            ...
            async with blob_service_client:
                ...
                blob_name = os.path.basename(f"{uuid.uuid4()}.png")
